"""批量跑指定 CVE 列表：build -> poc -> verify，结果写 JSON + 日志。用法: python batch_cves.py CVE-1 CVE-2 ..."""
...
CVES = sys.argv[1:] if len(sys.argv) > 1 else ["CVE-2021-36979", "CVE-2021-36980", "CVE-2021-38593",
...
def run_script(name, args):
    ...
    t0 = time.time()
    r = subprocess.run(cmd, cwd=str(source_root), env=env, capture_output=True, text=True, encoding="utf-8", errors="backslashreplace", timeout=7200)
    dt = time.time() - t0
    out = (r.stdout or "") + "\n" + (r.stderr or "")
...
for cve in CVES:
    ...
    try:
        ...
        if s["build"] is not True:
            ...
            row["detail"] = out[-800:]
            results.append(row)
            Path(source_root / "batch_cves_progress.json").write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
            continue
...
Path(source_root / "batch_cves_results.json").write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
print("BATCH DONE")
